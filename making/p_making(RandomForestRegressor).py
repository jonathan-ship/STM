...
# **RandomForestRegressor** 모델 훈련 시킴
from sklearn.ensemble import RandomForestRegressor
rnd_reg = RandomForestRegressor(n_estimators=500, max_leaf_nodes=16, random_state=42)
rnd_reg.fit(ri_MakingLT_prepared_train_re,ri_MakingLT_labels_train_re)
ri_MakingLT_predicted = rnd_reg.predict(ri_MakingLT_prepared_train_val)
print("Predictions:", ri_MakingLT_predicted)
print("Labels:", list(ri_MakingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
rnd_reg_mse_val = mean_squared_error(ri_MakingLT_labels_train_val, ri_MakingLT_predicted)
print(rnd_reg_mse_val)
...
plot_learning_curves_sup(rnd_reg, ri_MakingLT_prepared_train_re, ri_MakingLT_prepared_train_val, ri_MakingLT_labels_train_re, ri_MakingLT_labels_train_val)
...
rnd_reg_scores = cross_val_score(rnd_reg, ri_MakingLT_prepared_train,ri_MakingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
rnd_reg_rmse_scores = np.sqrt(-rnd_reg_scores)
...
print(display_scores(rnd_reg_rmse_scores))
...
param_grid = [
    {'n_estimators': [30,450,550,1000], 'max_depth': [15,25]}
  ]
grid = GridSearchCV(rnd_reg, param_grid, cv=5,
...
rnd_reg = RandomForestRegressor(max_depth=25,n_estimators=1000,random_state=42)
rnd_reg.fit(ri_MakingLT_prepared_train, ri_MakingLT_labels_train)
ri_MakingLT_predicted = rnd_reg.predict(ri_MakingLT_prepared_test)

from sklearn.metrics import mean_squared_error
rnd_reg_mse = mean_squared_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
rnd_reg_rmse = np.sqrt(rnd_reg_mse)
print(rnd_reg_rmse)

from sklearn.metrics import mean_absolute_error
rnd_reg_mae = mean_absolute_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
print(rnd_reg_mae)

rnd_reg_mape = (np.abs((ri_MakingLT_predicted - ri_MakingLT_labels_test) / ri_MakingLT_labels_test).mean(axis=0))
print(rnd_reg_mape)

from sklearn.metrics import mean_squared_log_error
rnd_reg_rmsle = np.sqrt(mean_squared_log_error(ri_MakingLT_labels_test, ri_MakingLT_predicted))
print(rnd_reg_rmsle)

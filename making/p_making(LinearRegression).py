...
lin_reg.fit(ri_MakingLT_prepared_train_re,ri_MakingLT_labels_train_re)
ri_MakingLT_predicted = lin_reg.predict(ri_MakingLT_prepared_train_val)
print("Predictions:", ri_MakingLT_predicted)
print("Labels:", list(ri_MakingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
lin_rmse_val = mean_squared_error(ri_MakingLT_labels_train_val, ri_MakingLT_predicted)
print(lin_rmse_val)
...
plot_learning_curves(lin_reg, ri_MakingLT_prepared_train_re, ri_MakingLT_prepared_train_val, ri_MakingLT_labels_train_re, ri_MakingLT_labels_train_val)
plt.axis([0, len(ri_MakingLT_prepared_train_re)/50, 0, 80000000000])
plt.show()


#훈렬한 모틸이 overfitting인지 unoverfitting인지 판단(방법2:교차검증)
from sklearn.model_selection import cross_val_score
lin_scores = cross_val_score(lin_reg, ri_MakingLT_prepared_train,ri_MakingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
...
lin_reg = LinearRegression()
param_grid = [{ "copy_X": [True, False], "fit_intercept": [True, False], "normalize": [True, False], "normalize": [True, False], "n_jobs": [None,-1,1,2,3,4,5]}]
...
lin_reg.fit(ri_MakingLT_prepared_train, ri_MakingLT_labels_train)
ri_MakingLT_predicted = lin_reg.predict(ri_MakingLT_prepared_test)

from sklearn.metrics import mean_squared_error
lin_mse = mean_squared_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
lin_rmse = np.sqrt(lin_mse)
print(lin_rmse)

from sklearn.metrics import mean_absolute_error
lin_mae = mean_absolute_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
print(lin_mae)

lin_mape = (np.abs((ri_MakingLT_predicted - ri_MakingLT_labels_test) / ri_MakingLT_labels_test).mean(axis=0))
print(lin_mape)

from sklearn.metrics import mean_squared_log_error
lin_rmsle = np.sqrt(mean_squared_log_error(ri_MakingLT_labels_test, ri_MakingLT_predicted))
print(lin_rmsle)

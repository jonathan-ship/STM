...
ada_reg.fit(ri_MakingLT_prepared_train_re,ri_MakingLT_labels_train_re)
ri_MakingLT_predicted = ada_reg.predict(ri_MakingLT_prepared_train_val)
print("Predictions:", ri_MakingLT_predicted)
print("Labels:", list(ri_MakingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
ada_reg_mse_val = mean_squared_error(ri_MakingLT_labels_train_val, ri_MakingLT_predicted)
...
plot_learning_curves_sup(ada_reg, ri_MakingLT_prepared_train_re, ri_MakingLT_prepared_train_val, ri_MakingLT_labels_train_re, ri_MakingLT_labels_train_val)
...
ada_reg_scores = cross_val_score(ada_reg, ri_MakingLT_prepared_train,ri_MakingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
...
ada_reg.fit(ri_MakingLT_prepared_train, ri_MakingLT_labels_train)
ri_MakingLT_predicted = ada_reg.predict(ri_MakingLT_prepared_test)

from sklearn.metrics import mean_squared_error
ada_reg_mse = mean_squared_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
ada_reg_rmse = np.sqrt(ada_reg_mse)
print(ada_reg_rmse)

from sklearn.metrics import mean_absolute_error
ada_reg_mae = mean_absolute_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
print(ada_reg_mae)

ada_reg_mape = (np.abs((ri_MakingLT_predicted - ri_MakingLT_labels_test) / ri_MakingLT_labels_test).mean(axis=0))
print(ada_reg_mape)


from sklearn.metrics import mean_squared_log_error
ada_reg_rmsle = np.sqrt(mean_squared_log_error(ri_MakingLT_labels_test, ri_MakingLT_predicted))
print(ada_reg_rmsle)

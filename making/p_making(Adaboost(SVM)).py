...
ada_svr_reg.fit(ri_MakingLT_prepared_train, ri_MakingLT_labels_train)
ri_MakingLT_predicted = ada_svr_reg.predict(ri_MakingLT_prepared_test)


from sklearn.metrics import mean_squared_error
ada_svr_reg_mse = mean_squared_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
ada_svr_reg_rmse = np.sqrt(ada_svr_reg_mse)
print(ada_svr_reg_rmse)

from sklearn.metrics import mean_absolute_error
ada_svr_reg_mae = mean_absolute_error(ri_MakingLT_labels_test, ri_MakingLT_predicted)
print(ada_svr_reg_mae)

ada_svr_reg_mape = (np.abs((ri_MakingLT_predicted - ri_MakingLT_labels_test) / ri_MakingLT_labels_test).mean(axis=0))
print(ada_svr_reg_mape)


from sklearn.metrics import mean_squared_log_error
ri_MakingLT_predicted[ri_MakingLT_predicted<0] = 0
ada_svr_reg_rmsle = np.sqrt(mean_squared_log_error(ri_MakingLT_labels_test, ri_MakingLT_predicted))

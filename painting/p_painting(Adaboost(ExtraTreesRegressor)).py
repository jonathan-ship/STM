...
ada_et_tree_reg.fit(ri_PaintingLT_prepared_train_re,ri_PaintingLT_labels_train_re)
ri_PaintingLT_predicted = ada_et_tree_reg.predict(ri_PaintingLT_prepared_train_val)
print("Predictions:", ri_PaintingLT_predicted)
print("Labels:", list(ri_PaintingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
ada_et_tree_reg_mse_val = mean_squared_error(ri_PaintingLT_labels_train_val, ri_PaintingLT_predicted)
...
plot_learning_curves_sup(ada_et_tree_reg, ri_PaintingLT_prepared_train_re, ri_PaintingLT_prepared_train_val, ri_PaintingLT_labels_train_re, ri_PaintingLT_labels_train_val)
plt.axis([0, len(ri_PaintingLT_prepared_train_re)/3000, 0, 20])
plt.show()

from sklearn.model_selection import cross_val_score
ada_reg_scores = cross_val_score(ada_et_tree_reg, ri_PaintingLT_prepared_train,ri_PaintingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
...
grid_result = ada_et_tree_reg_search.fit(ri_PaintingLT_prepared_train,ri_PaintingLT_labels_train)
...
ada_et_tree_reg = AdaBoostRegressor(base_estimator=ExtraTreeRegressor(ccp_alpha=0.0,
                  ...
                  random_state=42)
ada_et_tree_reg.fit(ri_PaintingLT_prepared_train, ri_PaintingLT_labels_train)
ri_PaintingLT_predicted = ada_et_tree_reg.predict(ri_PaintingLT_prepared_test)

from sklearn.metrics import mean_squared_error
ada_et_tree_reg_mse = mean_squared_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
ada_et_tree_reg_rmse = np.sqrt(ada_et_tree_reg_mse)
print(ada_et_tree_reg_rmse)

from sklearn.metrics import mean_absolute_error
ada_et_tree_reg_mae = mean_absolute_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
print(ada_et_tree_reg_mae)

ada_et_tree_reg_mape = (np.abs((ri_PaintingLT_predicted - ri_PaintingLT_labels_test) / ri_PaintingLT_labels_test).mean(axis=0))
print(ada_et_tree_reg_mape)


from sklearn.metrics import mean_squared_log_error
ada_et_tree_reg_rmsle = np.sqrt(mean_squared_log_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted))

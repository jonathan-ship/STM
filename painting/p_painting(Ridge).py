...
ridge_reg.fit(ri_PaintingLT_prepared_train_re,ri_PaintingLT_labels_train_re)
ri_PaintingLT_predicted = ridge_reg.predict(ri_PaintingLT_prepared_train_val)
print("Predictions:", ri_PaintingLT_predicted)
print("Labels:", list(ri_PaintingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
ridge_mse_val = mean_squared_error(ri_PaintingLT_labels_train_val, ri_PaintingLT_predicted)
print(ridge_mse_val)
...
plot_learning_curves(ridge_reg, ri_PaintingLT_prepared_train_re, ri_PaintingLT_prepared_train_val, ri_PaintingLT_labels_train_re, ri_PaintingLT_labels_train_val)
plt.axis([0, len(ri_PaintingLT_prepared_train_re)/50, 0, 20])
plt.show()

#훈렬한 모틸이 overfitting인지 unoverfitting인지 판단(방법2:교차검증)
from sklearn.model_selection import cross_val_score
ridge_scores = cross_val_score(ridge_reg, ri_PaintingLT_prepared_train,ri_PaintingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
...
ridge_reg = Ridge()
param_grid = [
  {'alpha': [0.001, 0.01, 0.1, 1, 10, 100, 1000], "fit_intercept": [True, False], "solver": ['svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga']}
 ]
...
ridge_reg = Ridge(alpha=10,fit_intercept=True,solver="saga")
ridge_reg.fit(ri_PaintingLT_prepared_train, ri_PaintingLT_labels_train)
ri_PaintingLT_predicted = ridge_reg.predict(ri_PaintingLT_prepared_test)

from sklearn.metrics import mean_squared_error
ridge_reg_mse = mean_squared_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
ridge_reg_rmse = np.sqrt(ridge_reg_mse)
print(ridge_reg_rmse)

from sklearn.metrics import mean_absolute_error
ridge_reg_mae = mean_absolute_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
print(ridge_reg_mae)

ridge_reg_mape = (np.abs((ri_PaintingLT_predicted - ri_PaintingLT_labels_test) / ri_PaintingLT_labels_test).mean(axis=0))
print(ridge_reg_mape)


from sklearn.metrics import mean_squared_log_error
ridge_reg_rmsle = np.sqrt(mean_squared_log_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted))

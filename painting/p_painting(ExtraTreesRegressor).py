...
Et_tree_reg = ExtraTreeRegressor(max_depth=10, random_state=42)
Et_tree_reg.fit(ri_PaintingLT_prepared_train_re,ri_PaintingLT_labels_train_re)
ri_PaintingLT_predicted = Et_tree_reg.predict(ri_PaintingLT_prepared_train_val)
print("Predictions:", ri_PaintingLT_predicted)
print("Labels:", list(ri_PaintingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
Et_tree_reg_mse_val = mean_squared_error(ri_PaintingLT_labels_train_val, ri_PaintingLT_predicted)
print(Et_tree_reg_mse_val)
...
plot_learning_curves(Et_tree_reg, ri_PaintingLT_prepared_train_re, ri_PaintingLT_prepared_train_val, ri_PaintingLT_labels_train_re, ri_PaintingLT_labels_train_val)
plt.axis([0, len(ri_PaintingLT_prepared_train_re)/50, 0, 20])
plt.show()

#훈렬한 모틸이 overfitting인지 unoverfitting인지 판단(방법2:교차검증)
from sklearn.model_selection import cross_val_score
Et_tree_reg_scores = cross_val_score(Et_tree_reg, ri_PaintingLT_prepared_train,ri_PaintingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
Et_tree_reg_rmse_scores = np.sqrt(-Et_tree_reg_scores)
...
print(display_scores(Et_tree_reg_rmse_scores))

#####GridSearch 방법을 이용해 최적 paramater 찾기
from sklearn.model_selection import GridSearchCV
Et_tree_reg = ExtraTreeRegressor(random_state=42)
param_grid = [
  { "max_depth":list(range(1, 100))}
 ]
grid = GridSearchCV(estimator=Et_tree_reg, param_grid=param_grid, verbose=2, cv=5,
...
from sklearn.metrics import mean_squared_error
Et_tree_reg_mse = mean_squared_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
Et_tree_reg_rmse = np.sqrt(Et_tree_reg_mse)
print(Et_tree_reg_rmse)

from sklearn.metrics import mean_absolute_error
Et_tree_reg_mae = mean_absolute_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
print(Et_tree_reg_mae)

Et_tree_reg_mape = (np.abs((ri_PaintingLT_predicted - ri_PaintingLT_labels_test) / ri_PaintingLT_labels_test).mean(axis=0))
print(Et_tree_reg_mape)


from sklearn.metrics import mean_squared_log_error
Et_tree_reg_rmsle = np.sqrt(mean_squared_log_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted))
print(Et_tree_reg_rmsle)

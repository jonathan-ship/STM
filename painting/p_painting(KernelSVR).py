...
svm_rbf_reg.fit(ri_PaintingLT_prepared_train_re,ri_PaintingLT_labels_train_re)
ri_PaintingLT_predicted = svm_rbf_reg.predict(ri_PaintingLT_prepared_train_val)
print("Predictions:", ri_PaintingLT_predicted)
print("Labels:", list(ri_PaintingLT_labels_train_val))
#rmse측정
from sklearn.metrics import mean_squared_error
svm_rbf_reg_mse_val = mean_squared_error(ri_PaintingLT_labels_train_val, ri_PaintingLT_predicted)
...
plot_learning_curves_sup(svm_rbf_reg, ri_PaintingLT_prepared_train_re, ri_PaintingLT_prepared_train_val, ri_PaintingLT_labels_train_re, ri_PaintingLT_labels_train_val)
...
svm_rbf_reg_scores = cross_val_score(svm_rbf_reg, ri_PaintingLT_prepared_train,ri_PaintingLT_labels_train,scoring="neg_mean_squared_error", cv=10)
...
grid_result = grid_search.fit(ri_PaintingLT_prepared_train,ri_PaintingLT_labels_train)
...
svm_rbf_reg = SVR(C=10, cache_size=200, coef0=0.0, degree=3,
                           ...
                           verbose=False)
svm_rbf_reg.fit(ri_PaintingLT_prepared_train, ri_PaintingLT_labels_train)
ri_PaintingLT_predicted = svm_rbf_reg.predict(ri_PaintingLT_prepared_test)

ri_PaintingLT_predicted[ri_PaintingLT_predicted<0] = 0
from sklearn.metrics import mean_squared_error
svm_rbf_reg_mse = mean_squared_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
svm_rbf_reg_rmse = np.sqrt(svm_rbf_reg_mse)
print(svm_rbf_reg_rmse)

from sklearn.metrics import mean_absolute_error
svm_rbf_reg_mae = mean_absolute_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted)
print(svm_rbf_reg_mae)

svm_rbf_reg_mape = (np.abs((ri_PaintingLT_predicted - ri_PaintingLT_labels_test) / ri_PaintingLT_labels_test).mean(axis=0))
print(svm_rbf_reg_mape)


from sklearn.metrics import mean_squared_log_error
svm_rbf_reg_rmsle = np.sqrt(mean_squared_log_error(ri_PaintingLT_labels_test, ri_PaintingLT_predicted))
